...
def getdata(x,y,norm=True,normdex=7,n=1000):
  if norm:
    ...
    ids=np.where(y!=normdex)
  qx=x[ids][:n]
  qy=np.reshape(qx,(int(qx.shape[0]),int(qx.shape[1]),int(qx.shape[2]),1))
...
def getmodel(q,d,reg=None,act="relu",mean=1.0,seed=None):
  np.random.seed(seed)
  tf.random.set_seed(seed)
  #inn=Input(shape=(28,28,1))
  inn=Input(shape=train.shape[1:])
  w=inn
  #w=inn
  #for a in qc:
  #  if a["typ"]=="conv":
  ...
  for aq in q[1:]:
    ...
    w=Conv2D(aq,3,kernel_initializer=keras.initializers.TruncatedNormal(),kernel_regularizer=reg,use_bias=False,activation=act)(w)
  w=Flatten()(w)
  for ad in d:
      w=Dense(ad,activation=act,use_bias=False,kernel_initializer=keras.initializers.TruncatedNormal(),kernel_regularizer=reg)(w)
...
l=[1,3,5,3,1]
d=[200,50,10]
#m=getmodel(l,d,reg=regulariser,act="relu")
mo=getmodel(l,d,reg=None,act="relu",mean=1.0,seed=seed)
mp=np.mean(mo.predict(t))
m=getmodel(l,d,reg=None,act="relu",mean=1.0,seed=seed)
print(m)
...
m=getmodel(l,d,reg=None,act="relu")
s=m.predict(t)
print(statinf(s))
print(statinf(pp))
